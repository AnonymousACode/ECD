...
def sum_except_batch(x):
    return x.view(x.size(0), x.size(1), -1).sum(-1)
...
class EnVariationalDiffusion(torch.nn.Module):
    ...
    def inflate_batch_array(self, array, target):
        ...
        target_shape = (array.size(0), array.size(1)) + (1,) * (len(target.size()) - 2)
    ...
    def kl_prior(self, xh, node_mask):
        ...
        ones = torch.ones((xh.size(0), xh.size(1), 1), device=xh.device)
    ...
    def sample_p_xh_given_z0(self, z0, node_mask, edge_mask, context, fix_noise=False):
        """Samples x ~ p(x|z0)."""
        zeros = torch.zeros(size=(z0.size(0), z0.size(1), 1), device=z0.device)
        gamma_0 = self.gamma(zeros)
    ...
    def compute_loss(self, x, h, node_mask, edge_mask, context, t0_always):
        ...
        if self.training and self.loss_type == 'l2':
            ...
            SNR_weight = (self.SNR(gamma_s - gamma_t) - 1).view(x.size(0), -1)
        ...
        if t0_always:
            ...
            loss = kl_prior + estimator_loss_terms + loss_term_0
            loss = loss.mean(dim=1)
            loss = loss + neg_log_constants
    ...
    def _generate_pyramid_scheduling_matrix(self, horizon: int, uncertainty_scale: float):
        sample_time = self.T + 1
        height = sample_time + int((horizon - 1) * uncertainty_scale) + 1
        scheduling_matrix = np.zeros((height, horizon), dtype=np.int64)
        for m in range(height):
            for t in range(horizon):
                scheduling_matrix[m, t] = sample_time + int(t * uncertainty_scale) - m

        return np.clip(scheduling_matrix, 0, sample_time) - 1
    ...
    #     curr_frame = 0
    #     n_frame = n_nodes
    #     # TODO (using the existing frame)
    #     z_curr = torch.tensor(data=[], dtype=z.dtype, device=z.device)
    #     edge_mask = edge_mask.view(n_samples, n_nodes, n_nodes)
    #     while curr_frame < n_frame:
    #         # TODO using longer horizon

    #         horizon = 1
    #         # scheduling_matrix = self._generate_scheduling_matrix(horizon)
    #         z_curr = torch.cat([z_curr, z[:, curr_frame:curr_frame + 1,:]], dim=1)

    #         for s in reversed(range(0, self.T)):
    #             t_array = torch.cat([torch.ones(n_samples, curr_frame, device=z.device), torch.full((n_samples, min(1, n_frame - curr_frame)), fill_value=s+1, device=z.device)], dim=1)
    #             s_array = torch.cat([torch.zeros(n_samples, curr_frame, device=z.device), torch.full((n_samples, min(1, n_frame - curr_frame)), fill_value=s, device=z.device)], dim=1)
    #             s_array = s_array / self.T
    #             t_array = t_array / self.T
    #             edge_mask_curr = edge_mask[:, :curr_frame+1, :curr_frame+1]
    #             z_curr = self.sample_p_zs_given_zt(s_array, t_array, z_curr, node_mask[:, :curr_frame+1, :], edge_mask_curr.reshape(-1, 1), context[:, :curr_frame+1, :], fix_noise=fix_noise)

    #         curr_frame += horizon
    #         # mid['position'].append(x)
    #         # mid['elements'].append(h['categorical'])

    #     # Finally sample p(x, h | z_0).
    #     x, h = self.sample_p_xh_given_z0(z_curr, node_mask, edge_mask.reshape(-1, 1), context, fix_noise=fix_noise)
    #     diffusion_utils.assert_mean_zero_with_mask(x, node_mask)
